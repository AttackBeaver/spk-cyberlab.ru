  ...
  return (
    <Layout>
      <div className="flex justify-between items-center mb-6 flex-wrap gap-2">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">✏️ Мои курсы (преподаватель)</h1>
      ...
      {error && <div className="text-red-500 dark:text-red-400 text-center py-8">{error}</div>}

      {showForm && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow dark:shadow-gray-700 mb-6 transition-colors duration-300">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">Новый курс</h2>
          <form onSubmit={handleCreateCourse} className="space-y-4">
            ...
            {formSuccess && <div className="text-green-500 dark:text-green-400 text-sm">{formSuccess}</div>}
            <button
              type="submit"
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.length === 0 ? (
            <p className="text-gray-500 dark:text-gray-400 col-span-full text-center">У вас пока нет курсов</p>
          ) : (
            courses.map((course) => (
              <div key={course.id} className="bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-gray-700 p-6 hover:shadow-lg transition-colors duration-300">
                <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-2">{course.title}</h3>
                <p className="text-gray-600 dark:text-gray-400 text-sm mb-4">{course.description || 'Нет описания'}</p>
